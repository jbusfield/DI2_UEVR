-- All code in this file courtesy of vinion
...
-- Set when widgets are embedded elsewhere (or showConfiguration is called).
-- Auto-create is deferred so the requiring file can claim them after require returns.
local configHandled = false
...
function M.getConfigurationWidgets()
    ...
    return spliceableInlineArray {
        ...
        { widgetType = "begin_group", id = "alxy_wheel_group", isHidden = true },
    ...
    configui.create({
        {
            ...
            layout = spliceableInlineArray {
                ...
                { widgetType = "new_line" },
                expandArray(M.getConfigurationWidgets),
            }
        }
    })
end

-- After the requiring script finishes: create a standalone panel only if widgets were never claimed.
setTimeout(0, function()
    if not configHandled then
        M.showConfiguration()
    end
end)
...
configui.onCreateOrUpdate("alyx_wheel_enabled", function(val)
    ...
    configui.setHidden("alxy_wheel_group", val ~= true)
end)
