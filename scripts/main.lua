...
-- TODO: 
-- Done - Heavy attack
-- Partially Done - Alternate immersion modes
-- Done - Better handling of claws, knuckles including animations
-- Done - Should heavy attack with claws use left trigger for left hand or will left trigger break something?
-- Alternate to left trigger to initiate throw in Full Immersion mode?
-- Cooldowns for weighted weapons
-- Investigate rendering issues
-- Done - The cutscene pawn may not exactly match where it should be in the scene. Investigate why
...
-- Disable input at OnCutsceneBegin (earlier than the detector) so mesh offsets are
-- cleared before the cinematic binds. input.lua clears XY when disable transitions.
-- local function setCutsceneInputDisabled(active)
-- 	active = active == true
-- 	status.cutsceneDisabledOverride = active or nil
-- 	if active then
-- 		if not input.isDisabled() then
-- 			status.cutsceneForcedInputDisable = true
-- 			input.setDisabled(true)
-- 		end
-- 	else
-- 		if status.cutsceneForcedInputDisable and status.knockdown ~= true then
-- 			input.setDisabled(false)
-- 		end
-- 		status.cutsceneForcedInputDisable = nil
-- 	end
-- end
...
uevrUtils.registerUEVRCallback("scaleform_ui_change", function(className, visible)
	...
	elseif className == "BP_HUDObject_Fader_C" then
		...
		-- 	end)
		-- end
	elseif className == "BP_HUDObject_Credits_C" or className == "BP_HUDObject_FailScreen_C" then
...
local function hideFader()
	pcall(function()
		...
		if uevrUtils.getValid(status.fader) == nil then return end
		local movie = status.fader:GetAsset("Fader")
		if uevrUtils.getValid(movie) == nil then return end
...
-- 	return status.cutsceneDisabledOverride, 10
-- end)

-- uevrUtils.registerCutsceneChangeCallback(function(isInCutscene)
-- 	--status.cutsceneDisabledOverride = isInCutscene or nil
-- 	--uevrUtils.setUEVRParam("VR_RoomscaleMovement", isInCutscene and "false" or "true")

-- -- 	if not isInCutscene then
-- -- 		uevrUtils.delay(3000, hideFader)
-- -- 	end
-- end)

--register_key_bind("F3", hideFader)

-- -- Unlimited health via SimpleHealthComponent god-mode (AddGodModeReason / bInvincible).
-- local godModeEnabled = false
-- local GOD_MODE_REASON = "UEVRDev"
-- local function getPlayerHealthComponent()
-- 	local pawn = uevrUtils.get_local_pawn()
-- 	if uevrUtils.getValid(pawn) == nil then return nil end
-- 	return uevrUtils.getValid(pawn.BPC_Player_SimpleHealth) or uevrUtils.getValid(pawn.HealthComponent)
-- end
-- local function setGodMode(enabled)
-- 	local hc = getPlayerHealthComponent()
-- 	if hc == nil then
-- 		print("[DI2] God mode: no HealthComponent")
-- 		return false
-- 	end
-- 	godModeEnabled = enabled == true
-- 	hc.bInvincible = godModeEnabled
-- 	if godModeEnabled then
-- 		pcall(function() hc:AddGodModeReason(GOD_MODE_REASON) end)
-- 		pcall(function()
-- 			if hc.MaxHealth ~= nil and hc.MaxHealth > 0 then
-- 				hc:SetHealth(hc.MaxHealth)
-- 			end
-- 		end)
-- 	else
-- 		pcall(function() hc:RemoveGodModeReason(GOD_MODE_REASON) end)
-- 	end
-- 	print("[DI2] God mode", godModeEnabled and "ON" or "OFF")
-- 	return true
-- end
...
gestures.registerBlockCallback(function(active, hand)
	--print("[DI2] gestures.registerBlockCallback", active, hand)
...
uevrUtils.registerOnPreInputGetStateCallback(function(retval, user_index, state)
	...
	if interactionControlMode == 3 or interactionControlMode == 4 and (ui.isRemapDisabled()) ~= true then
		...
		if xInputStatus.triggerEyes ~= triggerEar then
			xInputStatus.triggerEyes = triggerEar
			if triggerEyes then
